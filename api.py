...
# --- ADAPTIVE LAB-TIER PDF GENERATION LAYOUT ENGINE ---

def generate_pdf_report_bytes(protein_meta, ligand_summary, grid_box, pose_no, energy, interactions, full_df):
    """Compiles a complete publication-quality A4 PDF artifact via a flowing HTML schema."""
    try:
        from weasyprint import HTML
    except ImportError:
        return None # Gracefully switch off if binary layers aren't initialized locally
    ...
    for idx, row in full_df.iterrows():
        ...
        <tr>
            ...
            <td>{row['RMSD l.b.']}</td>
            <td>{row['RMSD u.b.']}</td>
            <td style="font-size: 11px;">{row['Interacting Residues']}</td>
        </tr>
        """
    ...
    if not interactions_html:
        interactions_html = "<li>No atomic connections tracked under 3.8 Å radius bounds.</li>"
    ...
    <head>
        ...
        <style>
            @page {{
                ...
                @bottom-center {{
                    content: "Report generated by InSilico BioSphere | Developer: Mr. Sarang S. Dhote, Assistant Professor, Department of Chemistry, Shivaji Science College, Nagpur, India | Contact: sarangreserach@gmail.com";
    ...
    <body>
        <div class="header-banner">
            ...
            <p>High-Throughput Molecular Docking & Biophysical Analysis Report</p>
        </div>
        
        <p style="text-align: right; font-size: 10pt; color: #718096;"><b>Date:</b> {datetime.now().strftime('%B %d, %Y')}</p>
        
        <h2>1. Screening Experiment Overview</h2>
        <div class="summary-block">
            <p style="margin: 0;">In silico molecular docking run exploring the thermodynamic structural affinity profiles matching target protein macromolecule constraints against parsed compound configuration records.</p>
        </div>

        <h2>2. Macromolecular Target Protein Matrix</h2>
        <table>
            <tr><td style="width: 30%;"><b>Target Identifier:</b></td><td><code>{protein_meta['id']}</code></td></tr>
            <tr><td><b>Structural Title:</b></td><td>{protein_meta['title']}</td></tr>
            <tr><td><b>Experimental Organism:</b></td><td><i>{protein_meta['organism']}</i></td></tr>
            <tr><td><b>Method & Resolution:</b></td><td>{protein_meta['method']} ({protein_meta['res']})</td></tr>
        </table>

        <h2>3. Small Molecule Drug Ligand Matrix</h2>
        <table>
            <tr><td style="width: 30%;"><b>PubChem Profile:</b></td><td>{ligand_summary.replace('**','')}</td></tr>
        </table>

        <h2>4. Bounding Grid Search Box Configuration</h2>
        <table>
            <tr><td style="width: 30%;"><b>Center Coordinates (X, Y, Z):</b></td><td>({grid_box['cx']}, {grid_box['cy']}, {grid_box['cz']})</td></tr>
            <tr><td><b>Grid Box Dimensions (Size):</b></td><td>{grid_box['sx']} Å × {grid_box['sy']} Å × {grid_box['sz']} Å</td></tr>
            <tr><td><b>Search Exhaustiveness:</b></td><td>{grid_box['ex']}</td></tr>
        </table>

        <div style="page-break-before: always;"></div>

        <h2>5. Profile Core Mode Pose Results</h2>
        <div class="kpi-container">
            <span style="font-size: 11pt; color: #2c5282; font-weight: bold; text-transform: uppercase;">Selected Fit: Mode {pose_no} Pose Alignment</span><br>
            <span class="kpi-val">{energy} kcal/mol</span>
        </div>

        <h2>6. Binding Site Amino Acid Contacts Breakdown</h2>
        <ul>
            {interactions_html}
        </ul>

        <h2>7. Global Binding Affinity Screening Rankings Sheet</h2>
        <table>
            <thead>
                <tr>
                    <th>Binding Mode</th>
                    ...
                    <th>RMSD l.b.</th>
                    <th>RMSD u.b.</th>
                    <th>Interacting Amino Acid Residues</th>
                </tr>
            </thead>
            <tbody>
                {table_rows_html}
...
# --- MASTER ENVIRONMENT CLEAR COUPLING ---
if st.button("🔄 Reset Entire Environment for Fresh Docking", type="secondary", use_container_width=True):
    ...
    st.success("Dashboard cache and runtime structural templates cleared completely!")
    st.rerun()
...
if st.session_state.docking_results_raw is not None:
    ...
    if not df_results.empty:
        ...
        with col_export:
            ...
            # --- DYNAMIC EXECUTABLE PRINT COMPILER ---
            try:
                p_meta = extract_pdb_metadata(st.session_state.local_target_path, st.session_state.pdb_id_display) if st.session_state.local_target_path else {"id": st.session_state.pdb_id_display, "title": "Custom Vector", "organism": "N/A", "method": "N/A", "res": "N/A"}
                grid_params = {"cx": grid_cx, "cy": grid_cy, "cz": grid_cz, "sx": grid_sx, "sy": grid_sy, "sz": grid_sz, "ex": exhaustiveness}
                active_pose_no = int(st.session_state.get('selected_pose_idx', 1)) if 'selected_pose_idx' in st.session_state else 1
                active_pose_energy = get_pose_affinity(st.session_state.docking_results_raw, active_pose_no)
                active_ints = compute_spatial_interactions("protein.pdbqt", split_docking_poses("docking_poses.pdbqt").get(active_pose_no, ""))
                
                pdf_bytes = generate_pdf_report_bytes(
                    protein_meta=p_meta,
                    ligand_summary=st.session_state.ligand_summary_text,
                    grid_box=grid_params,
                    pose_no=active_pose_no,
                    energy=active_pose_energy,
                    interactions=active_ints,
                    full_df=df_results
                )
                if pdf_bytes:
                    st.download_button(label="📝 Download PDF Analytical Report Sheet", data=pdf_bytes, file_name="insilico_biosphere_report.pdf", mime="application/pdf", use_container_width=True)
            except Exception as pdf_err:
                st.sidebar.info("PDF extension ready. Install WeasyPrint to enable document compiling.")
