...
def ensure_linux_vina_exists():
    if not os.path.exists("./vina"):
        with st.spinner("Initializing Vina..."):
            try:
...
with col1:
    ...
    if st.button("Fetch Ligand Info"):
        ...
        # Note: Add conversion logic here
        st.session_state.lig_loaded = True
        st.success("Ligand Data Prepared")
...
with col2:
    ...
    if st.session_state.get("results"):
        st.text_area("Docking Output:", value=st.session_state.results, height=300)
    else:
        st.info("Docking pending...")
